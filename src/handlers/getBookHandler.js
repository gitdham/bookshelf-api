...
const getBookHandler = (request, h) => {
  ...
  const indexBooks = books
    ...
    .filter((book) => {
      ...
      if (reading === '1' || reading === '0') return (reading === '1' && book.reading) || (reading === '0' && !book.reading)
      return book
    })
    .filter((book) => {
      // if finished is 1 or 0 filter books by finished status
      if (finished === '1' || finished === '0') return (finished === '1' && book.finished) || (finished === '0' && !book.finished)
      return book
    })
